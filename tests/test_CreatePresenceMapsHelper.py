...
class CreatePresenceMapsTestCase(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.cpm = CreatePresenceMapHelper(self.oh, self.gh, self.ch, self.verbose)
    ...
    def test_convert_spatial_to_image(self):
        src, profile, new_band = self.cpm.open_band()
        presence_data, lon, lat = self.cpm.extract_lat_lon('testspecies1')
        presence_data = presence_data.append(pd.DataFrame(
            [[1, 2, 3, 10, 80, 6, 1], [1, 2, 3, -14, 26, 6, 1], [1, 2, 3, 44, -143, 6, 1], [1, 2, 3, 12, 0, 6, 1]],
            columns=list(presence_data.columns)))
