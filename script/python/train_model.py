...
def train_model (path):
    ...
    for species in taxa["taxon"][:]:
        ...
        variables.remove("%s presence map.tif"%spec.replace('_',' ')) #drop species own occurrences from from variable list
    
        table = pd.read_csv(path +"/data/spec_ppa_env/%s_env_dataframe.csv"%spec)
        table=  table.drop('%s presence map.tif'%spec.replace('_',' '),axis=1) #drop species own occurrences from features
