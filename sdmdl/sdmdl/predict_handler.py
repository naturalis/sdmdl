...
class predict_handler():
    ...
    def predict_model(self):
        ...
        colors = [[norm(0), "0.95"],[norm(0.05),"steelblue"],[norm(0.1),"sienna"],[norm(0.3),"wheat"],[norm(0.5),"cornsilk"],[norm(0.95),"yellowgreen"],    [norm(1.0),"green"]]                
        ...
        for species in tqdm.tqdm(self.oh.name,desc = 'Predicting globally'+(31 * ' ')) if self.verbose else self.oh.name:        
            spec=species
            ...
            with rasterio.open(self.ch.result_path + '/{}/{}_predicted_map.tif'.format(spec,spec), 'w', **profile) as dst:
