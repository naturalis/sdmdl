...
class data_prep_handler():
    ...
    def raster_stack_clip(self):
        ...
        for key in (tqdm.tqdm(self.oh.spec_dict,desc='Creating raster clips' + (29 *' ')) if self.verbose else self.oh.spec_dict):  
    ...
    def calc_band_mean_and_stddev(self):
        ...
        with open(self.gh.gis + '/env_bio_mean_std.txt','w+') as file:
            ...
            file.close()
        for i in (tqdm.tqdm(range(1,self.gh.scaled_len+1),desc='Computing band means and standard deviations' + (6 * ' ')) if self.verbose else range(1,self.gh.scaled_len+1)):
