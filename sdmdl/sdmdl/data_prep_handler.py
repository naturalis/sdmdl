...
class data_prep_handler():
    ...
    def __init__(self,occurrence_handler,gis_handler,verbose):
        ...
        self.calc_band_mean_and_stddev()
        self.create_training_df()
    ...
    def calc_band_mean_and_stddev(self):
        ...
        with open(self.gis_handler.gis + '/env_bio_mean_std.txt','w+') as file:
            file.write("band"+"\t"+"mean"+"\t"+"std_dev"+"\n")
            file.close()
            
        for i in (tqdm.tqdm(range(1,self.gis_handler.length),desc='Computing band means and standard deviations' + (6 * ' ')) if self.verbose else range(1,self.gis_handler.length)):
            ...
            #write to file
            with open(self.gis_handler.gis + '/env_bio_mean_std.txt','a') as file:
                file.write(str(i)+"\t"+str(mean)+"\t"+str(std_dev)+"\n")
                
    def create_training_df(self):
        
        src=rasterio.open(self.gis_handler.stack + '/stacked_env_variables.tif')
        inRas=gdal.Open(self.gis_handler.stack + '/stacked_env_variables.tif')        
        for i in (tqdm.tqdm(self.occurrence_handler.name,desc='Creating training dataframe' + (23 * ' ')) if self.verbose else self.occurrence_handler.name):            
            data = pd.read_csv(self.gis_handler.spec_ppa + '/%s_ppa_dataframe.csv'%i)
            ...
            mean_std=pd.read_csv(self.gis_handler.gis + '/env_bio_mean_std.txt',sep="\t")
            mean_std=mean_std.to_numpy()
            X=[]
            species =["%s"%spec]*int(len(row))        
            for j in range(0,self.occurrence_handler.length):
                band=myarray[j]
                x=[]
                for i in range(0,len(row)):
                    value= band[row[i],col[i]]
                    if j < self.gis_handler.scaled_len:
                        if value <-1000:
                            value=np.nan
                        else:  
            ...
            df["present/pseudo_absent"]=ppa
            df["row_n"]=row
            df.rename(columns=dict(zip(df.columns[0:self.occurrence_handler.length], self.occurrence_handler.name)),inplace=True)
            df=df.dropna(axis=0, how='any')
            input_data=df
            if not os.path.isdir(self.gis_handler.spec_ppa_env):
                os.makedirs(self.gis_handler.spec_ppa_env,exist_ok=True)
            input_data.to_csv(self.gis_handler.spec_ppa_env + '/%s_env_dataframe.csv'%spec)
    ...
    def create_prediction_df(self):
        ...
        inRas=gdal.Open(self.gis_handler.stack + '/stacked_env_variables.tif')
        myarray=inRas.ReadAsArray()
        df=pd.read_csv(self.gis_handler.gis + '/world_locations_to_predict.csv')        
        ...
        df=df.dropna(axis=0, how='any')
        df.head()
        input_X=df.iloc[:,0:self.occurrence_handler.length]
        np.shape(input_X)
        row=df[self.occurrence_handler.length]
        col=df[self.occurrence_handler.length+1]
        row_col=pd.DataFrame({"row":row,"col":col})
        input_X=input_X.values
        row=row.values
        col=col.values
        np.save(self.gis_handler.gis + '/world_prediction_array.npy',input_X)
        row_col.to_csv(self.gis_handler.gis + '/world_prediction_row_col.csv')
        if self.verbose:
            print(' Done!')
