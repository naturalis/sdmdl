...
class train_handler():
    ...
    def train_model(self):
        ...
        """training the deep learning models and writing the resulting models to file."""
        ...
        for species in tqdm.tqdm(self.oh.name, desc='training models' + (35 * ' ')) if self.verbose else self.oh.name:
            ...
            Best_model_AUC = [0]
            for i in (
            tqdm.tqdm(range(1, 6), desc='%s' % spec + ((50 - len(spec)) * ' ')) if self.verbose else range(1, 6)):
                btchs = 75
                num_classes = 2
                epch = 150
                num_inputs = X.shape[1]
                model = Sequential()
                layer_1 = Dense(250, activation='relu',
                                input_shape=(num_inputs,))  # , kernel_regularizer=regularizers.l2(0.000001))
                layer_2 = Dense(200, activation='relu',
                                input_shape=(num_inputs,))  # , kernel_regularizer=regularizers.l2(0.000001))
                layer_3 = Dense(150, activation='relu',
                                input_shape=(num_inputs,))  # , kernel_regularizer=regularizers.l2(0.0000001))
                layer_4 = Dense(100, activation='relu',
                                input_shape=(num_inputs,))  # , kernel_regularizer=regularizers.l2(0.00000001))
                model.add(layer_1)
                ...
                training_generator, steps_per_epoch = balanced_batch_generator(X_train, y_train, sampler=NearMiss(),
                                                                               batch_size=btchs, random_state=42)
                model.fit_generator(generator=training_generator, steps_per_epoch=steps_per_epoch, epochs=epch,
                                    verbose=0)
                ...
                if AUC > Best_model_AUC[0]:
                    ...
                    if int(len(X_train)) > 5000:
                        ...
                        test_set.rename(columns=dict(zip(test_set.columns[0:len(self.gh.names) - 1], variables)),
                                        inplace=True)
                        shap_values = explainer.shap_values(shuffled_X_test)
                        shap.summary_plot(shap_values[1], test_set, show=False)
                        plt.savefig(self.ch.result_path + '/{}/{}_feature_impact'.format(spec, spec),
                                    bbox_inches="tight")
                        plt.close()
                    else:
            ...
            with open(self.ch.result_path + '/_DNN_performance/DNN_eval.txt', 'a') as file:
                file.write(spec + "\t" + str(avg_loss) + "\t" + str(avg_acc) + "\t" + str(avg_tpr) + "\t" + str(
                    avg_AUC) + "\t" + str(avg_lci) + "\t" + str(avg_uci) + "\t" + str(occ_len) + "\t" + str(
                    abs_len) + "\n")
