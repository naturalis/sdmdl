...
class PresencePseudoAbsenceHelper:
    ...
    def __init__(self, oh, gh, ch, verbose):
        ...
        np_random = 1 #set to self.ch.np_r
        np.random.seed(np_random)

    def draw_random_absence(self, r):
        presence_data = self.oh.spec_dict[self.key]
        ...
        self.outer_random_sample_lon_lats = lon_lat_array[
                                            np.random.choice(lon_lat_array.shape[0], self.random_sample_size,
                                                             replace=False), :]
    ...
    def create_presence_pseudo_absence(self):
        r = gdal.Open(self.stack_path)

        for self.key in (tqdm.tqdm(self.oh.spec_dict,
                                   desc='Sampling pseudo absence' + (27 * ' ')) if self.verbose else self.oh.spec_dict):
            presence_data = self.draw_random_absence(r)
